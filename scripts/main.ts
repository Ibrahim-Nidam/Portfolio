 ...
 */
function initRotatingRoles(): void {
    ...
    function getCurrentLanguage(): 'en' | 'fr' {
        return (document.documentElement.lang || 'en') as 'en' | 'fr';
    }
 ...
 */
function initSkillsSection(): void {
    const skillsData = [
        // Frontend
    ...
    // Render skills
    function renderSkills(category: string = 'all'): void {
...
function initContactForm(): void {
    ...
    if (form) {
        form.addEventListener('submit', (e) => {
            ...
            const mailtoLink = `mailto:ibrahim.nidam22@gmail.com?subject=${encodeURIComponent(data.subject as string)}&body=${encodeURIComponent(`From: ${data.name} (${data.email})\n\n${data.message}`)}`;
 ...
 * Download CV file
 */
function downloadCV(lang: 'en' | 'fr'): void {
